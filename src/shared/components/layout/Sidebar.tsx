...
import { currentFY, getInitials } from '@/shared/utils/format';
...
function SidebarContent({ activeView, onNavigate, onClose }: { activeView: ViewId; onNavigate: (v: ViewId) => void; onClose?: () => void }) {
  const { dark, toggleTheme, profile } = useAppStore();
...
export default function Sidebar({ activeView, onNavigate, open, onOpenChange }: SidebarProps) {
    ...
    return (
      <Sheet open={open} onOpenChange={onOpenChange}>
        <SheetContent side="left" className="w-[270px] p-0 ledger-sidebar border-r-0">
          <SidebarContent activeView={activeView} onNavigate={onNavigate} onClose={() => onOpenChange?.(false)} />
        </SheetContent>
      </Sheet>
    );
  }
