        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th className="!text-right !pr-5">Credit</th>
            </tr>
          </thead>
          <tbody>
            {accounts.map((a, i) => {
              if ('section' in a && a.section) {
                return (
                  <tr key={i} className="bg-background">
                    <td colSpan={5} className="!pl-5 font-bold text-muted-foreground text-[10.5px] tracking-widest">
                      {a.section}
                    </td>
                  </tr>
                );
              ...
              return (
                <tr key={i}>
                  ...
                  <td>{acc.name}</td>
                  <td>
                    <span className={`badge-status ${typeClass(acc.type)}`}>
                  ...
                  <td className={`!text-right ${
                    acc.debit > 0
                      ? (acc.type === 'Expense' ? 'text-destructive' : 'text-success')
                      : 'text-muted-foreground'
                  } font-semibold`}>
                    {acc.debit > 0 ? val(acc.debit) : '—'}
                  </td>
                  <td className="!text-right !pr-5 text-success font-semibold">
                    {acc.credit > 0 ? val(acc.credit) : <span className="text-muted-foreground">—</span>}
                  </td>
                </tr>
              );
            })}
