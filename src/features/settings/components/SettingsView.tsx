  ...
  return (
    <div>
      <div className="flex items-end justify-between mb-5">
        ...
        <button
          onClick={handleSave}
          className="inline-flex items-center gap-1.5 px-3 py-[7px] rounded-lg text-[12.5px] font-medium bg-primary text-primary-foreground cursor-pointer hover:opacity-90 transition-opacity"
        >
          Save Changes
        </button>
      </div>
      ...
      <div className="bg-card border border-border rounded-xl p-5 mb-4">
        ...
        <div className="space-y-3">
          {[
            ...
            <div
              key={f.label}
              className="grid grid-cols-[180px_1fr] items-center gap-3.5 py-3 border-b border-border last:border-0"
            >
              <div>
                <label className="text-[13px] font-medium">{f.label}</label>
                <div className="text-[11.5px] text-muted-foreground mt-0.5">{f.sub}</div>
              </div>
              <input
                value={f.value}
                onChange={e => f.onChange(e.target.value)}
                placeholder={f.placeholder}
                className="border border-border rounded-lg px-3 py-[7px] text-[13px] bg-background outline-none focus:border-primary transition-colors w-full"
              />
            </div>
          ))}
        </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-[180px_1fr] items-center gap-3.5 py-3 border-b border-border">
            <div>
              ...
              <div className="text-[11.5px] text-muted-foreground mt-0.5">Used in invoices and reports</div>
            </div>
            <input
              value={biz}
      ...
      <div className="bg-card border border-border rounded-xl p-5 mb-4">
        ...
        <div className="grid grid-cols-[180px_1fr] items-center gap-3.5 py-3 border-b border-border">
          <div>
            <label className="text-[13px] font-medium">Encryption Passcode</label>
            <div className="text-[11.5px] text-muted-foreground mt-0.5">Encrypts all data in localStorage</div>
          </div>
        ...
        <div className="grid grid-cols-[180px_1fr] items-center gap-3.5 py-3">
          <div>
            <label className="text-[13px] font-medium">Export Data</label>
            <div className="text-[11.5px] text-muted-foreground mt-0.5">Download a JSON backup</div>
          </div>
          <button
            onClick={exportData}
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        >
          ...
          <div className="text-xs text-destructive/80 mb-3.5">
            This will permanently delete all invoices, expenses, clients, vendors, and notifications.
            Your profile settings will be kept.
          </div>
          <button
            onClick={handleReset}
            className="px-3 py-[7px] rounded-lg text-[12.5px] font-medium bg-destructive text-destructive-foreground cursor-pointer hover:opacity-90 transition-opacity"
          >
