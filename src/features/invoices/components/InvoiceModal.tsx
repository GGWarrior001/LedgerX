...
import { addInvoice } from '@/features/invoices/services/invoiceService';
...
export default function InvoiceModal({ onClose }: Props) {
  ...
  const { pushSnapshot, pushEntry } = useCloudSync();
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    addInvoice(
      ...
      {
        onCloudSync: pushSnapshot,
        onPushEntry: (inv) => pushEntry('invoice', inv),
      },
    );
    onClose();
  };
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box w-[480px]">
        ...
        <form onSubmit={handleSubmit} className="p-5">
          ...
          <div className="form-field"><label>Description</label><input type="text" name="description" placeholder="e.g. Web Design Project" required /></div>
          <div className="grid grid-cols-2 gap-2.5">
            <div className="form-field"><label>Issue Date</label><input type="date" name="issueDate" defaultValue={today} required /></div>
            <div className="form-field"><label>Due Date</label><input type="date" name="dueDate" defaultValue={due} required /></div>
          </div>
          <div className="grid grid-cols-2 gap-2.5">
            <div className={`form-field ${errors.amount ? 'error' : ''}`}>
              <label>Amount</label><input type="number" name="amount" min="1" placeholder="50000" required />
              {errors.amount && <span className="text-[11px] text-destructive">{errors.amount}</span>}
            </div>
            <div className="form-field"><label>Status</label><select name="status"><option value="draft">Draft</option><option value="sent">Sent</option></select></div>
          </div>
          <div className="flex justify-end gap-2 mt-4">
            ...
            <button type="submit" className="px-3 py-2 rounded-lg text-[12.5px] font-medium bg-primary text-primary-foreground cursor-pointer hover:opacity-90 transition-opacity">Create Invoice</button>
          </div>
        </form>
      </div>
    </div>
