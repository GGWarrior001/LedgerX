    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {EXPENSE_CATEGORIES.map(cat => (
          <div
            key={cat.name}
            className="bg-card border border-border rounded-xl p-3.5 text-center cursor-pointer transition-all hover:border-primary hover:-translate-y-px"
          >
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <div className="px-5 pt-[18px]">
          <div className="text-[13.5px] font-semibold">All Expenses</div>
        </div>
          ...
          <tbody>
            {expenses.length === 0 ? (
              ...
              <tr key={e.id}>
                ...
                <td>
                  <span className="badge-status" style={{ background: 'rgba(99,102,241,0.1)', color: '#6366F1' }}>
                    {e.category}
                  </span>
                </td>
                <td className="text-muted-foreground">{e.vendor}</td>
                <td className="text-muted-foreground">{fmtDate(e.date)}</td>
                <td className={`text-xs font-medium ${e.receipt === 'attached' ? 'text-success' : 'text-warning'}`}>
                  {e.receipt === 'attached' ? '✓ Attached' : '⚠ Pending'}
                </td>
                <td className="!text-right !pr-5 font-semibold tabular-nums text-destructive">
