import { addClient } from '@/features/clients/services/clientService';
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box w-[440px]">
        ...
        <form onSubmit={handleSubmit} className="p-5">
          <div className="form-field"><label>Company Name</label><input type="text" name="name" placeholder="e.g. Acme Corp" required /></div>
          <div className="grid grid-cols-2 gap-2.5">
            <div className="form-field"><label>City</label><input type="text" name="city" placeholder="e.g. Bangalore" required /></div>
            <div className="form-field"><label>Email</label><input type="email" name="email" placeholder="contact@company.com" required /></div>
          </div>
          ...
          <div className="flex justify-end gap-2 mt-4">
            ...
            <button type="submit" className="px-3 py-2 rounded-lg text-[12.5px] font-medium bg-primary text-primary-foreground cursor-pointer hover:opacity-90 transition-opacity">Add Client</button>
          </div>
        </form>
      </div>
    </div>
