    ...
    <div>
      <div className="flex items-end justify-between mb-5">
        ...
        <div className="flex gap-2">
          <button
            onClick={exportCSV}
            className="inline-flex items-center gap-1.5 px-3 py-[7px] rounded-lg text-[12.5px] font-medium bg-background border border-border cursor-pointer hover:bg-muted transition-colors"
          >
            Download CSV
          </button>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3.5 mb-5">
        <div className="bg-card border border-border rounded-xl p-5" style={{ borderTop: '3px solid hsl(var(--primary))' }}>
          ...
          <div className="text-[22px] font-bold tracking-tight tabular-nums mb-0.5">
            {netProfit < 0 ? '−' : ''}{val(netProfit)}
          </div>
          <div className={`text-xs font-medium ${netProfit >= 0 ? 'text-success' : 'text-destructive'}`}>
            {netProfit >= 0 ? 'Net Profit' : 'Net Loss'} · {Math.abs(margin)}% margin
          </div>
        </div>
        ...
        <table className="data-table">
          <thead>
            <tr>
              ...
              <th className="!text-right">Full Year</th>
            </tr>
          </thead>
          <tbody>
            <tr className="font-semibold">
              <td>Total Revenue</td>
              {qRev.map((v, i) => (
                <td key={i} className="!text-right text-success">{privacyMode ? '***' : `${cs}${fmt(v)}`}</td>
              ))}
            ...
            <tr className="font-semibold">
              ...
              {qExp.map((v, i) => (
                <td key={i} className="!text-right text-destructive">{privacyMode ? '***' : `${cs}${fmt(v)}`}</td>
              ))}
            ...
            <tr className="font-bold bg-primary/5">
              ...
              {qProfit.map((v, i) => (
                <td key={i} className={`!text-right !border-t-2 !border-border ${v < 0 ? 'text-destructive' : 'text-success'}`}>
                  {privacyMode ? '***' : `${v < 0 ? '−' : ''}${cs}${fmt(Math.abs(v))}`}
                </td>
              ))}
              <td className={`!text-right !border-t-2 !border-border ${netProfit < 0 ? 'text-destructive' : 'text-success'}`}>
                {privacyMode ? '***' : `${netProfit < 0 ? '−' : ''}${cs}${fmt(Math.abs(netProfit))}`}
              </td>
            </tr>
          </tbody>
        </table>
