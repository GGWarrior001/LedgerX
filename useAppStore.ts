 ...
 *   - App-lock state and encryption (min 6-char passcode enforced)
 *   - In-app notifications (derived from invoice statuses)
 *   - isHydrating flag for cloud-sync loading feedback
 */
...
interface AppStoreState {
  ...
  /**
   * Sets up encryption with a user-supplied passcode.
   * @throws {Error} if the passcode is fewer than 6 characters.
   */
...
export const useAppStore = create<AppStoreState>((set, get) => ({
  ...
  /** Returns the currency symbol from the current profile, defaulting to ₹ */
  ...
  lock: () => {
    // Phase 3: Explicitly clear in-memory key on lock to prevent leaks
  ...
  setupEncryption: (passcode) => {
    // Phase 3: Enforce minimum 6-character passcode
