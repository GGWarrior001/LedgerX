 ...
 *
 * Phase 2: `syncToCloud` is debounced via a 1.5-second timer to prevent
 * rapid successive Firestore writes during quick mutations (e.g. bulk imports).
 */
 ...
 * Phase 2: Debounced by 1.5 s — rapid successive calls are collapsed into one write.
 */
export function syncToCloud(uid: string): void {
  ...
  syncTimer = setTimeout(() => {
    ...
    saveCloudData(uid, data).catch(err => {
      console.error('[LedgerX] Failed to push cloud snapshot:', err);
    });
  }, 1500);
}
