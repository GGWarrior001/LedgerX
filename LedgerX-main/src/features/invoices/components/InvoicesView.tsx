...
const TABS: FilterKey[] = ['all', 'draft', 'sent', 'paid', 'overdue'];
...
export default function InvoicesView() {
  ...
  invoices.forEach(i => { if (i.status in counts) (counts as Record<string, number>)[i.status]++; });
    ...
    <div>
      ...
      <div className="flex gap-0.5 mb-3.5 bg-background p-0.5 rounded-lg w-fit">
        {TABS.map(t => (
          <div
            key={t}
            className={`px-3 py-1.5 rounded-md text-[12.5px] font-medium cursor-pointer transition-all ${
              filter === t ? 'bg-card font-medium shadow-sm' : 'text-muted-foreground hover:text-foreground'
            }`}
            onClick={() => setFilter(t)}
          >
            {t.charAt(0).toUpperCase() + t.slice(1)} ({(counts as Record<string, number>)[t]})
          </div>
        ))}
      </div>
          ...
          <tbody>
            {list.length === 0 ? (
              ...
              <tr key={i.id}>
                ...
                <td>
                  <div className="flex items-center gap-2.5">
                    <div
                      ...
                      style={{ background: i.clientColor, color: '#fff' }}
                    >
                      {i.clientInitials}
                    </div>
                ...
                <td className={`!text-right !pr-5 font-semibold tabular-nums ${
                  i.status === 'paid' ? 'text-success' : i.status === 'overdue' ? 'text-destructive' : ''
                }`}>
