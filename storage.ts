...
class StorageService {
  // Phase 3: Explicitly typed as null when cleared to prevent key leakage
  ...
  /**
   * Derives and stores the encryption key from the given passcode.
   * NOTE: Minimum passcode length (6 chars) is enforced in useAppStore.setupEncryption.
   * This method does NOT re-validate length to keep crypto core untouched.
   */
  ...
  /**
   * Phase 3: Explicitly wipes the in-memory key.
   * Called from useAppStore.lock() and authService.logOut() to prevent
   * the key from lingering in memory after a session lock or sign-out.
   */
